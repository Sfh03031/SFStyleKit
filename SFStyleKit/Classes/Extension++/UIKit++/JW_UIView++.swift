...
public extension UIView {
    ...
    /// - Parameters:
    ///   - size: badge point size, default = `CGSize(width: 8, height: 8)`
    ///   - top: top margin between badge and superview, default = `0.0`
    ///   - right: right margin between badge and superview, default = `0.0`
    func showBadgePoint(_ size: CGSize = CGSize(width: 8, height: 8), top: CGFloat = 0.0, right: CGFloat = 0.0) {
        let badge = UIView(frame: CGRect(x: self.frame.size.width - size.width - right, y: top, width: size.width, height: size.height))
        badge.backgroundColor = UIColor.hexColor(hex: "#FF3030")
